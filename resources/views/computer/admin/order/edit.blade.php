...
<div id="page-wrapper">
    ...
    @endif
    <!-- /.row -->
    <!-- Thong tin khach hang vaf don hang -->
    <div class="row info @if($order->order_status==1)alert-warning @elseif($order->order_status==2) alert-info @elseif($order->order_status==3) alert-success @else alert-danger @endif" style="padding: 15px 0;">         
           <div class="col-xs-6">                        
              <div class="form-group">
                <label for="nn-name-cus" class="col-xs-3 control-label"><i class="fa fa-id-badge"></i> {{ trans("admin.custommer_o") }}:</label>
                <div class="col-xs-7">
                    <p><b>{{ $order->name }}</b></p>
                </div>
              </div>
              ...
              <div class="form-group">
                <label for="nn-key-cus" class="col-xs-3 control-label"><i class="fa fa-money"></i> {{ trans("admin.total_price") }}:</label>
                <div class="col-xs-8" >
                    <span id="nnshowtotal">
           ...
           <div class="col-xs-6">
            {{-- Hinh thuc thanh toan --}}
                <div class="form-group">
                    <label for="nn-phone-cus" class="col-sm-4 control-label"><i class="fa fa-clock-o"></i> {{ trans("admin.payment") }}:</label>
                    <div class="col-sm-6">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                      <p>   + {{ trans("admin.address") }}:{{ $order->address }} <br>
                            + {{ trans("admin.phone_shipping") }}:{{ $order->shipping_telephone }} <br>
                            + {{ trans("admin.shipping") }} :{{ $order->shipping_name }} <br>
                ...
                {{-- Tong chi phi (ca tien van chuyen) --}}
                {{-- <div class="form-group">
                  <label for="nn-phone-cus" class="col-xs-4 control-label"><i class="fa fa-ravelry"></i> {{ trans("admin.total_pay_with_shipping") }}:</label>
                  <div class="col-xs-8" >
                    <p>{{ $order->total_pay  ? $order->total_pay : '###############'}}</p>
                  </div>
                </div>  --}}   

                <div class="form-group">
                <label for="nn-key-cus" class="col-xs-4 control-label"><i class="fa fa-money"></i> {{ trans("admin.total_pay_with_shipping") }}:</label>
                <div class="col-xs-8" >
                    <span id="nnshowtotal">
                      {{ number_format($order->total_pay ? $order->total_pay : '0' ) }}
                    </span>                        
                        <input type="hidden" name="nntotalorder" id="nntotalorder" value="{{ $order->total_pay }}" />
                </div>
              </div>            
           </div>   
    </div>
    <!-- /.Thong tin khach hang vaf don hang -->
    <!-- Chi tiet don hang so luong gia  -->
    <div class="row"> 
        <div class="col-xs-12">
             <!-- /.panel-heading -->
          <div class="col-lg-12">
            ...
            <table width="100%" class="table table-striped table-bordered table-hover">
                ...
                @foreach($detail as $de)
                    <tr>
                        ...
                        <td>{{ $de->total }}</td>
                        <td>{{ $de->note }}</td>
                        @if($order->order_status!=3)<td class="center"><span class="nndelorderdetail btn btn-danger" iddetailpro="{{ $de->id }}" total="{{ $de->total }}" ><i class="fa fa-trash"></i> {{ trans("admin.delete") }}</span></td>@endif
                    </tr>
                @endforeach
                </tbody>
            </table>
