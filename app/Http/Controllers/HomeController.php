...
class HomeController extends Controller {
    ...
    public function public_var(){
            $modproducts_product    = ModProduct::select('modproducts.*', DB::raw('translates.trname as trname'))
            ...
            foreach ($modproducts_product as $itemmod => $valuemod) {
                $listproducts_product   = ListProduct::select('listproducts.*', DB::raw('translates.trname as trname'))
                ...
                $modproducts_product[$itemmod]["listproducts_product"] = $listproducts_product; 
            }


            $list_special_view_index_products   =  Product::select('*', DB::raw('products.id as idproduct') )
            ...
            $result = array(
                ...
                "socials"=>$socials,
                "languages"=>$languages,
                "modproducts_product"=>$modproducts_product,
                "list_special_view_index_products"=>$list_special_view_index_products,
	...
	public function submit_order(Request $request){
		if(Cart::count() > 0){
            ...
            $p_shipping_address = $request->input('p_shipping_address');
            $p_pay_address = $request->input('p_pay_address');
            ...
            // $sltCountpeople     = $request->input('sltCountpeople');
              ...
              Session::put('pay_address', session('pay_address'));

              Session::put('shipping_name', session('pay_name'));
              Session::put('shipping_phone', session('pay_phone'));
              Session::put('shipping_address', session('shipping_address'));
              // Session::put('shipping_address', session('pay_address'));
            ...
            // Find payment_name, shipping_name of shippings and payments table
			$shipping 		= Shippings::find($idshipping);
			$payment 		= Payments::find($idpayment);


	        $item_order = new Order;
			if(session('logined_cusid') != "" && session('logined_cus') == 1){ //-- khách đăng nhập(có tài khoản)
                ...
                // $item_order->payment_address            = $p_pay_address;
                // $item_order->shipping_address           = $p_shipping_address;

	        }else{	//--- Khách thanh toán không có tài khoản
	        	$item_order->user_name  = "Khách thanh toán không có tài khoản";
            ...
            $item_order->payment_name               = $payment->name;
            ...
            $item_order->shipping_name              = $shipping->name;
            ...
            // $item_order->payment_address            = $this->store_address_shipping();
            // $item_order->shipping_address           = $this->store_address_pay();
